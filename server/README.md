...
__*Tip__: if your application name is, for example, __advanced__, to see the api side of it you 
...
9. To run tests written for api side of your application 
   ```cd``` to ```_protected/tests/codeception/api``` , run ```codecept build``` and then run your tests.
...
_protected
    ...
    common
        ...
        models/              contains model classes used in both backend and api
...
themes                   contains api themes
uploads                  contains various files that can be used by both api and backend applications

```
Version 2.2.0 changes
-------------------
1) Adds `uploads` folder to the application root that can be shared by both api and backend applications.  
